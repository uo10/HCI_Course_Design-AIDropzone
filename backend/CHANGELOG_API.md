...
> **版本**: v1.0 → v1.1  
> **日期**: 2026-05-25 → 2026-05-28  
> **完整契约**: 见 `backend/API_CONTRACT.md`

---

## v1.1.1 热修复 (2026-05-28)

### 修复：导出/Tag 库扫不到文件

**问题**: 手动放入 workspace 的文件（或在标签索引上线前已有文件）没有索引条目，
`GET /tags` 和 `POST /export` 无法发现它们。

**修复**:
- 新增 `POST /tags/rescan` 显式重建索引接口
- `GET /tags` 和 `POST /export` 现在**自动扫描** workspace，
  补录未被索引的文件（从文件名提取已知标签词）
- 已通过 `/rename` 入库的标签**不会被覆盖**

### 改进：Mock AI 现在真正读取文件

`POST /parse` 现在会打开磁盘上的实际文件，计算真实的 SHA-256 哈希值
和文件大小，并将其写入 `suggested_name` 的 `{hash}` 部分和 `summary` 末尾
（`文件大小 53B，SHA256=86a222479adc`）。
