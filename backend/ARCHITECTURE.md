...
四大模块各司其职：AI 解析识别文件内容与标签（含敏感文件拦截），重命名模块按模板改写文件名并将旧路径写入回滚日志，导出模块按标签筛选文件并打包为 zip，回滚模块提供任意时刻的撤销能力。
...
## 核心机制四：敏感文件拦截 — 隐私保护第一道闸门

### 设计动机

在真实办公场景中，用户拖入的文件可能包含个人身份信息（身份证扫描件、简历）、商业机密（合同）或医疗隐私（病历）。在 AI 解析阶段即进行敏感文件识别，可在文件进入后续流程之前就发出警告，避免敏感信息被无意识地重命名传播或打包分享。
...
| 角色 | 行为 |
|------|------|
| **后端 (Mock AI)** | 文件名关键词匹配 → 自动追加 `sensitive` 标签 + 警告 summary |
| **前端 (GUI)** | 检测到 `sensitive` 标签 → 红色高亮该文件卡片 + 弹窗确认"此文件疑似包含敏感信息，是否继续操作？" |
| **未来 (真 LLM)** | 模型提示词接管语义级识别（如无关键词但内容涉密的 PDF），实现更深层的隐私扫描 |
...
> 该机制在 Mock AI 解析器中以 **零延迟、零成本** 的方式运行——仅做字符串包含判断，无正则回溯、无网络调用。
> 标记为 `sensitive` 后，前端可依据此标签触发用户二次确认，构成"**AI 预警 + 人工复核**"的双层防护体系。
...
│   ├── mock_ai_parser.py         # Mock 解析 — 扩展名分类 + 关键词 + 敏感文件拦截
...
- **敏感文件预警**：AI 解析层内置关键词拦截（身份证、简历、合同等），命中自动标记 `sensitive` 标签提醒前端。后续可升级为真 LLM 语义级隐私识别。
